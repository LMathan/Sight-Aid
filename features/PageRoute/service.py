...
class QueryService:
    ...
    def Store(self, user_data: dict,pages : PagesModel):
        ...
        new_query = QueryModel(id=None, query=validated_data.query, language=validated_data.language,response=None,itemorplace=None)
...
class AgentService:
    ...
    def item_agent_info(self):
        return """Use "item function" to find the place or item they looking for."""
    ...
    def run_agent(self, queryModel:QueryModel) -> QueryModel:
        ...
        self.client.run(
            agent=router_agent,
            messages=[{"role": "system", "content": "Provide the page the user looking for"},
        ...
        item_name_agent = Agent(
            name="Router Agent",
            model="qwen2.5-coder:3b",
            instructions=self.item_agent_info(),
            functions=[self.item]
        )
        self.client.run(
            agent=item_name_agent,
            messages=[{"role": "system", "content": "Provide the item or place the user looking for"},
                      {"role": "user", "content": f"{query}"}],
            # context_variables=None,
            stream=False,
        )

        queryModel.itemorplace=self.itemOrPlace

        # self.client.run(
        #     agent=router_agent,
        #     messages=[{"role": "system", "content": "Provide the item or place name the user looking for"},
        #               {"role": "user", "content": f"{query}"}],
        #     # context_variables=None,
        #     stream=False,
        # )
